...
//solo la variable que se usa antes del punto ".". EJ: rect1.area(), rect1 sería &self
                      ...
                      //*******Funciones Asociadas*******//
//estas funciones no tienen lo de &self como parametro de entrada, por lo que no son métodos
  fn cuadrado(dimension : u32) -> Self{ //esta fnción crea un cuadrado
    Self { //el Self es un alias para la estructura a la que estamos implementando está
      //función (o cualquiera dentro de este scope de impl)
      largo : dimension,
      ancho : dimension, //para invocar este tipo de funciones asociadas, se usa el ::
      //así como cuando creamos una string y le vamos a colocar algo con String::from("algo")
    }
  }
}

//Nota no tan importante (por ahora, hasta el chapter 10, supongo):
//pueden hacerse multiples bloques de impl, no hay ningún problema con eso,
//aunque no haya ninguna razón para hacerlo (por ahora) es una sintaxis valida qque el lenguaje
//acepta
fn main() {
  ...
  //aquí vamos a crear un cuadrado con la función asociada que creamos en la implementación
