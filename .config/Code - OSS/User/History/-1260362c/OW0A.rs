...
//rust también nos deja retornar multiples tuplas (no muy relevante la vdd, ya se sabía. Pero lo coloco pq aparece en el libro)
//y no cae mal echarle un ojo
    let (s2, len) = calculate_length(s1); //s1 es moved, por lo cual pierde ownership, s2 gana esa ownership
    //digamos q s2 está expropiando a  s1. ahora s1 queda invalidado, no se puede usar
