...
fn main() {
  ...
  //Ejemplos usando Option, el enum de prelude con el que podemos definir valores presentes
  //o ausentes (Some es presente, none es ausente)
  let some_number = Some(5);
  let some_char : char = Some('e');
...
impl Message {
  fn call(&self) {
    if let Message::Write(string) = &self {
      println!("Contenido del mensaje: {}", string);
    } else {
      println!("Mensaje no es de tipo Write.");
    }
  }
}
